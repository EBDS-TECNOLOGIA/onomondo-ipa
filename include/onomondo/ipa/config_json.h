...
struct ipa_run_config {
	...
	struct {
		...
		/*! Log level for every subsystem (JSON: log.level, "error", "info" or "debug"), applied by
		 *  ipa_run() before it logs anything; -1 when not configured, which leaves the level as the
		 *  front end set it. */
		int level;

		/*! Per-subsystem log levels (JSON: log.subsys_levels, an object mapping a subsystem name to a
		 *  level), applied after level. -1 for a subsystem that is not listed. */
		int subsys_level[_NUM_LOG_SUBSYS];
	} log;

	/*! True when the file carries a "platform" object. That object belongs to the platform around the
	 *  IPAd -- on OpenWrt, the readiness and event scripts read it -- and the parser only checks that it is
	 *  an object. */
	bool has_platform;
};
