...
/* See onomondo/ipa/run_observer.h. */
static ipa_run_observer_cb observer;
static void *observer_priv;
...
static void notify(struct ipa_context *ctx, enum ipa_run_event ev, int rc)
{
	if (observer)
		observer(ctx, ev, rc, observer_priv);
}

/* Whether the nvstate has to be written back. On a router it lives on flash, and most runs end with exactly the
 * state they started with, so writing it every time would wear the flash for nothing. This relies on the same
 * state always serializing to the same bytes, which nvstate_serialize() in ipad.c guarantees. */
static bool nvstate_changed(const struct ipa_buf *before, const struct ipa_buf *after)
{
	if (!before)
		return true;
	return before->len != after->data_len || memcmp(before->data, after->data, after->data_len) != 0;
}
...
static int poll_loop(struct ipa_context *ctx, bool one_euicc_pkg_only)
{
	...
	while (!run_stop) {
		...
		rc = ipa_poll(ctx);
		notify(ctx, IPA_RUN_EV_POLLED, rc);
...
int ipa_run(struct ipa_run_config *rcfg)
{
	...
	/* Log levels from the configuration take effect before the first line is logged. */
	if (rcfg->log.level >= 0)
		ipa_log_set_level_all(rcfg->log.level);
	for (i = 0; i < _NUM_LOG_SUBSYS; i++) {
		if (rcfg->log.subsys_level[i] >= 0)
			ipa_log_set_level(i, rcfg->log.subsys_level[i]);
	}
	...
	notify(ctx, IPA_RUN_EV_INITIALIZED, 0);
	...
	if (rcfg->initial_eim_cfg_path) {
		...
		/* Same subsets as the CLI's -m: everything but the Provisioning Profiles, which carry the
		 * bootstrap connectivity a deployed device needs to reach its eIM again. */
		...
		notify(ctx, IPA_RUN_EV_EIM_READY, 0);
...
leave:
	...
	if (nvstate_save && !nvstate_changed(nvstate_load, nvstate_save)) {
		IPA_LOGP(SMAIN, LDEBUG, "nvstate unchanged, not rewriting %s\n", rcfg->nvstate_path);
	} else if (nvstate_save) {
