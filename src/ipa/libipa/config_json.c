...
void ipa_run_config_defaults(struct ipa_run_config *rcfg)
{
	...
	/* No log level configured: the front end's choice stands. */
	rcfg->log.level = -1;
	for (i = 0; i < _NUM_LOG_SUBSYS; i++)
		rcfg->log.subsys_level[i] = -1;
}
...
static const char *const known_keys[] = {
	...
	"platform",		/* (no flag; settings of the platform around the IPAd, not interpreted here) */
};
...
static int get_log_level(json_t *val, const char *path, int *out)
{
	...
	if (!val)
		return 0;
	if (!json_is_string(val) || (level = ipa_log_level_by_name(json_string_value(val))) < 0) {
		IPA_LOGP(SMAIN, LERROR, "config: \"%s\" must be one of \"error\", \"info\", \"debug\"\n", path);
		return -EINVAL;
	}
	*out = level;
	return 0;
...
/* "subsys_levels": { "ES10x": "debug", ... } */
static int get_subsys_levels(json_t *obj, struct ipa_run_config *rcfg)
{
	const char *key;
	json_t *val;
	int subsys;
	...
	if (!json_is_object(obj)) {
		IPA_LOGP(SMAIN, LERROR, "config: \"log.subsys_levels\" must be an object\n");
		return -EINVAL;
	}
	...
	json_object_foreach(obj, key, val) {
		if (key_is_comment(key))
			continue;
		subsys = ipa_log_subsys_by_name(key);
		if (subsys < 0) {
			IPA_LOGP(SMAIN, LERROR, "config: unknown log subsystem \"log.subsys_levels.%s\"\n", key);
			return -EINVAL;
		}
		snprintf(path, sizeof(path), "log.subsys_levels.%s", key);
		if (get_log_level(val, path, &rcfg->log.subsys_level[subsys]) < 0)
			return -EINVAL;
	}
	return 0;
}

/* The "platform" object is the platform's own; only its type is checked here. */
static int get_platform(json_t *obj, struct ipa_run_config *rcfg)
{
	json_t *val = json_object_get(obj, "platform");

	if (!val)
		return 0;
	if (!json_is_object(val)) {
		IPA_LOGP(SMAIN, LERROR, "config: \"platform\" must be an object\n");
		return -EINVAL;
	}
	rcfg->has_platform = true;
	return 0;
}
...
{
	static const char *const log_keys[] = { "path", "max_size_bytes", "max_files", "level", "subsys_levels" };
	...
	if (get_log_level(json_object_get(log, "level"), "log.level", &rcfg->log.level) < 0)
		return -EINVAL;
	if (get_subsys_levels(json_object_get(log, "subsys_levels"), rcfg) < 0)
		return -EINVAL;
