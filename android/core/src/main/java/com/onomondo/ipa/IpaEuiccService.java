 ...
 *
 * Because the framework never binds this service, every callback is a no-op
 * returning a "nothing here" value. This deliberately makes the device's real
 * eSIM UI non-functional while installed -- acceptable on a dedicated device,
 * and the reason installing this app is not a neutral act.
 *
 * EuiccService is a @SystemApi absent from the public SDK, so this file is
 * compiled against core/libs/euicc-system-stubs.jar (compileOnly; the real
 * classes come from the framework at runtime and are never packaged).
 */
public final class IpaEuiccService extends EuiccService {
