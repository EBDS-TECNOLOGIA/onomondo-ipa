...
/**
 * The seam that keeps device-specific setup out of the reusable spike harness.
 *
 ...
 * [DeviceProfileProvider] on its Application. The spike UI ([SpikeActivity])
 * asks the Application for a profile and falls back to [GenericDeviceProfile]
 * (stock AOSP) when none is provided -- so the harness never references any
 * vendor library.
 */
...
/**
 * Implemented by an Application to hand its [DeviceProfile] to the spike UI.
 * Keeping this on the Application (rather than a static registry) is what lets a
 * device module own its setup without the harness importing it.
 */
